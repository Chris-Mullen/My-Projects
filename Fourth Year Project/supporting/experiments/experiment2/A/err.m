...
mkvid=true,lossless=true,play=true,framesout=false,correlation=false,drawfigs=false)
	%% mkblock='a' means create all new blocks of data, should only be used for first use so that all blocks, 
	...
	%% framesout=true means decompress the frames from the resulting mp4 video files, this is used
	%%	to test the recording quality of ffmpeg, an absolute pixel difference of zero means that the frame was 
	%%	recorded perfectly
	%% correlation=true means the cross correlation between the blocks and the final product will be checked.
	...
	if(mkblock=='a')
		...
		if((ans=input("press \'y\' to continue, anything else to exit\n","s"))=="y")
